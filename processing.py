...
def process_ecs(file: str) -> pd.DataFrame:
    ...
    otus = df["OTU_ID"]
    df["Enzyme"] = otus.str.split(":").str[0].str.split("|").str[0]
    df["Genus"] = otus.str.split("|").str[1]
    df["Genus"] = otus.str.split(".").str[0]
    # df.drop(df[df["Genus"].isnull()].index, inplace=True)
    df.drop(df[df["Genus"] == "unclassified"].index, inplace=True)
...
if __name__ == "__main__":
    logging.basicConfig(filename='microbiome.log', level=logging.INFO)
    main()
